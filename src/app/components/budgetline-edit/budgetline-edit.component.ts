...
@Component({
  selector: 'app-budgetline-edit',
  templateUrl: './budgetline-edit.component.html',
  styleUrls: ['./budgetline-edit.component.css']
})
export class BudgetlineEditComponent implements OnInit {
  ...
  for (const p in this.docheader){
      ...
      this.docheader.VALUEOF = this.budgetview.BASE_VALUE
    ...
    checkValue(doc:any){
      return ( doc.BUDGETGROUP.length > 4 )
    }
    updateSAP() {
     this.vm.NOTE = this.apiserv.xtdbtoa( this.vm.NOTE)
     this.vm.TITLE = this.apiserv.xtdbtoa( this.vm.TITLE )
     this.vm.ENTRYCODE = 'CREATE'
     this.apiserv.postGEN(this.vm, 'PUT_CIPLINE_MVT').subscribe(reply => {
        this.apiserv.messagesBS.next('Created')
      })
